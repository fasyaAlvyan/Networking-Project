...
Proyek ini merupakan simulasi jaringan untuk ketersediaan tinggi dengan mengkombinasikan/mengintegrasikan protokol seperti *Open Shortest Path First(**OSPF**), Virtual Router Redundancy Protocol(**VRRP**), Rapid Spanning Tree Protocol(**RSTP**) dan Link Aggregation Control Protocol(**LACP**)*
...
- Kondisi saat dua client melakukan ping menuju IP publik *1.1.1.1* ![kondisi ping dan pembagian beban](https://github.com/fasyaAlvyan/Networking-Project/blob/main/networking-project/Multi-Protocol%20High%20Availability%20LAN%20(OSPF%2C%20VRRP%2C%20RSTP%2C%20LACP)/Screenshot/Loadbalancing%20aktif.png) Saat dua client di subnet yang sama, client Office 1 dengan IP *192.168.10.3* dan client office 2 dengan IP *192.168.10.10* melakukan ping ke alamat IP publik yang sama, dan dari dua panel *wireshark* pada gambar, terdeteksi kedua IP client office berjalan di dua link yaitu link eth1 dan eth2 pada switch acces 2, dapat disimpulkan jika algoritma XoR yang dipakai LAG untuk menghasilkan hash yanng menentukan link yang akan digunakan packet, bekerja dengan baik dengan menyebarkan packet ke berbagai link di dalam groupnya secara berurutan, Sama halnya saat pengetesan kapasitas bandwidht pada dua device(SWL3 Dist dan SWL3 Dist 2) pada hasil pengujian ini,![Bandwidht test](https://github.com/fasyaAlvyan/Networking-Project/blob/main/networking-project/Multi-Protocol%20High%20Availability%20LAN%20(OSPF%2C%20VRRP%2C%20RSTP%2C%20LACP)/Screenshot/Bandwidth%20test.png) pada gambar terdeksi jika kapasitas bandwidth hanya kurang dari 1 Mbps saja, walaupun di dalam bundle LACP terdapat 2 link berkapasitas 1 Mbps, itu karena mekanisme load balancing yang digunakan LAG bergantung pada algoritma XoR yang dipakainya yang di mana untuk satu jenis koneksi selalu menghasilkan 1 link yang akan dipakai, dan keunikan koneksi ini bergantung pada pemilihan *transmit-hash-policy* yang digunakan, contohnya saat koneksi client ke sebuah server di LAN, sebaiknya menggunakan kombinasi *layer 2 dan 3* karena LAG akan melakukan perhitungan XoR dengan alamat src/dst MAC address atau IP agar menghasilkan kombinasi pemilihan link yang unik dan seimbang, atau contoh lainnya seperti koneksi client ke internet, sebaiknya menggunakan kombinasi *layer 3 dan 4* karena LAG akan menghitungan berdasarkan alamat src/dst IP dan port.

### 5. Pengecekan Fail over OSPF
- Kondisi saat link antar R-core dan Switch Dist 1 belum putus ![Link belum putus](https://github.com/fasyaAlvyan/Networking-Project/blob/main/networking-project/Multi-Protocol%20High%20Availability%20LAN%20(OSPF%2C%20VRRP%2C%20RSTP%2C%20LACP)/Screenshot/Saat%20Link%20belum%20putus(OSPF).png) saat link belum putus terlihat pada tabel routing milik Switch Dist 1 masih menggunakan default route yang menuju gateway 10.0.1.1 pada link LAG-to-R_Core. Lalu pada konektivitas client Office, ping masih berjalan dan mendapatkan *ICMP echo reply* ![Hasil ping dari client menuju IP publik](https://github.com/fasyaAlvyan/Networking-Project/blob/main/networking-project/Multi-Protocol%20High%20Availability%20LAN%20(OSPF%2C%20VRRP%2C%20RSTP%2C%20LACP)/Screenshot/Saat%20link%20belum%20putus(test%20OSPF).png).
- Kondisi saat link antar R-core dan Switch Dist 1 putus ![Link putus](https://github.com/fasyaAlvyan/Networking-Project/blob/main/networking-project/Multi-Protocol%20High%20Availability%20LAN%20(OSPF%2C%20VRRP%2C%20RSTP%2C%20LACP)/Screenshot/Saat%20link%20putus(Test%20OSPF1).png) saat link putus terlihat pada tabel routing milik Switch Dist 1 mengalami perubahan, default gateway yang tadinya menuju IP miliki R-core dan link menuju R-Core berubah dan menggunakan IP milik switch dist 2 dengan link menuju switch tersebut, pada sisi client terlihat banyak perubahan, ![Sisi client saat link putus](https://github.com/fasyaAlvyan/Networking-Project/blob/main/networking-project/Multi-Protocol%20High%20Availability%20LAN%20(OSPF%2C%20VRRP%2C%20RSTP%2C%20LACP)/Screenshot/Saat%20link%20putus(Test%20OSPF).png) terlihat pada sisi client jika ada 1x time out dan 1x destination reachble, itu karena protokol OSPF melakukan kalkulasi ulang dengan algoritma SPFnya agar link kembali aktif, bukti lainnya ada pada hasil *ICMP Echo Reply* yang didapat jika TTL yang sebelumnya bernilai 126, menjadi 125 karena path packet ICMP berubah karena harus melewati R-core(TTL 127) ---> Switch Dist 2 (TTL 126) ---> Switch Dist 1 (TTL 125).
...
- Kekurangan kelima pada topologi ini adalah firewall yang belum didesain dengan baik dan masih menggunakan firewall pada RouterOS yang berbasis stateless(setiap koneksi akan di accept oleh perangkat by default).
### Kelebihan
- Kelebihan pertama pada topologi ini adalah Ketersediaan yang tinggi di scope jaringan local.
- Kelebihan kedua pada topologi ini adalah tingkat toleransi kesalahan yang tinggi, jika ada kegagalan pada jaringan, mekanisme pada protokol yang digunakan akan bekerja untuk memperbaiki atau menghandle kesalahan yang terjadi.
...
- Kelebihan keempat pada topologi ini adalah fleksibilitas dan modularitas, jaringan ini sangat mudah untuk diskalable tanpa harus menganggu device di jaringan.
...
- kelebihan keenamp pada topologi ini adalah mudah untuk di managed oleh administator jaringan.
